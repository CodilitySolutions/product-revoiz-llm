...
def interview_simulation(llm_client, beginning_message):
    messages = []
    print(f"Agent: {beginning_message}")
    messages.append({"role": "assistant", "content": beginning_message})
    ...
    while True:
        request = {
            "response_id": 0,
            "transcript": messages,
            "interaction_type": "normal",  # assuming normal interaction initially
        }

        response = llm_client.draft_response(request)
        if "The interview is over" in response["content"]:
...
interview_simulation(llm_client, begin_sentence)